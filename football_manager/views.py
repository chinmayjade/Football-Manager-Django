...
def approachToBuy(request):
    if request.method == "POST":
        ...
        Transfer.objects.create(price=request.POST['offered_price'], player=this_player, buyer_club=this_user.club, seller_club=this_player.team) 
...
def approveOffer(request):
    if request.method == "POST":
        ...
        Transfer_Archived.objects.create(price=this_transfer.price, approved=True, player_first_name=this_transfer.player.first_name, player_last_name=this_transfer.player.last_name, buyer_club_name=this_transfer.buyer_club.name, seller_club_name=this_transfer.seller_club.name)
...
def pullPlayersData(request):
    ...
    for team in all_teams:
        ...
        for p in players['api']['players']:
            ...
            Player.objects.create(first_name=p['firstname'], last_name=p['lastname'], age=p['age'], position=p['position'], country=p['nationality'], team=team)
